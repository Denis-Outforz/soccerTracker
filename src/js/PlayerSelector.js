...
const PlayerSelector = ({
  ...
  const handlePlayerDoubleClick = (team, playerNumber) => {
    const newNumber = prompt(
      `Enter new player number for ${team} player ${playerNumber}:`,
      playerNumber,
    )
  ...
  return (
    <div className="player-selector-container">
      <div className="team-container">
        <button
          className={`team-button home-button ${
            selectedPlayer && selectedPlayer.startsWith('Home') ? 'selected' : ''
          }`}
        ...
        <button
          className={`team-button away-button ${
            selectedPlayer && selectedPlayer.startsWith('Away') ? 'selected' : ''
          }`}
      ...
      <div className="team-container">
        <div className="player-list">
          {homePlayers.map((playerNumber, index) => (
            <div
              key={`home-${index + 1}`}
              className={`player home-player ${
                selectedPlayer === `Home${playerNumber}` ? 'selected' : ''
              }`}
        ...
        <div className="player-list">
          {awayPlayers.map((playerNumber, index) => (
            <div
              key={`away-${index + 1}`}
              className={`player away-player ${
                selectedPlayer === `Away${playerNumber}` ? 'selected' : ''
              }`}
